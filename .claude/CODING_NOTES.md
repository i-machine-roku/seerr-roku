...
- `DashboardTask.brs::fetchApi()` used synchronous `GetToString()` with no timeout, called up to 5x per tab. Bounded now via `ApiTask.brs`'s async+`wait()`+cancel pattern — a real bug, but not the cause of the boot-splash hangs (see next note).
- **Root cause of all boot-splash hangs: an `<interface>` `<function>` needs `callFunc("name")`, not a direct call** — dot-call only resolves fields. `showLogin()` called `m.loginView.screenShown()` directly, crashing every launch. Found via emulator tracing.
- **Standalone `TextEditBox` doesn't work for real text entry on Roku hardware** — renders/focuses fine, but OK/Select never enters edit mode. Use a focusable control/row that opens `KeyboardDialog` on OK instead — confirmed on real hardware here and in upstream PR #1.
- Local emulator testing (`brs-node` CLI + `brs-engine`/`brs-scenegraph` in-browser) caught the real callFunc crash above, missed entirely across multiple static-review passes — worth setting up despite the gotchas below.
- Emulator gotchas (1/2): stale bundled `common.zip` in the browser build (copy `node_modules/brs-node/assets/common.zip` over it on `ENOENT .../fonts/system-fonts.json`); both zips need HTTP `Range` support or fetches silently corrupt.
- Emulator gotchas (2/2): `app.zip`/`common.zip` need real cache-busting (`no-store` + query param, or clearing browser site data); the SceneGraph extension path in `brs.initialize()` is worker-relative (`"./brs-sg.js"`, not `"./lib/brs-sg.js"`).
- `brs-scenegraph`'s `Button` never sets `buttonSelected` on OK/Select (no key handling at all) — a standalone `Button`'s dialog flow can't be verified in-emulator, needs real hardware regardless.
- A literal `--` inside an XML comment body is illegal per spec and fails the *entire* component's parse with "Malformed comment" — `CreateObject` then silently returns `invalid`. Use a real em dash (`—`) instead of `--` in any BrightScript XML comment.
- Roku's default `Button` draws **no background at all when unfocused** (only set when focused or `showFocusFootprint` is true) — without it, an unfocused button reads as plain text with a stray bullet. Set `showFocusFootprint="true"` for a clickable-at-rest look.
