...
- The above fix alone wasn't sufficient — the same "scroll down then up before anything selects" symptom persisted. Root cause: a screen's own `init()` runs during `CreateObject()`, *before* its caller's `appendChild` attaches it to the live scene tree, and `setFocus(true)` called that early doesn't reliably register with the platform focus manager. Fix: defer the initial `setFocus()` one tick via a zero-duration `Timer`'s `fire` field (see `LoginScreen.brs::onInitialFocusTimer`) instead of calling it directly in `init()`. Any other screen with this pattern (`grep -rn "setFocus(true)" components/Screens/`) should get the same treatment if it shows the same symptom.
