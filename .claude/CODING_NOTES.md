...
- Crossfading between two images: use two `Poster`s + one `Animation` with two `FloatFieldInterpolator`s (one 1→0, one 0→1), and set each interpolator's `fieldToInterp` to `"<posterId>.opacity"` **at runtime** before starting the animation, not fixed in XML — the two Posters swap which one is "active" every cycle, so the target field has to be reassigned each time (see `LoginScreen.brs::showNextBackdrop`). Stop any repeating `Timer` driving this explicitly when the screen is done (e.g. on login success) — a detached node doesn't necessarily stop its own timers.
- **Don't copy a source app's animation timing onto a TV without reconsidering it.** Seerr's own web login rotates backdrops every 6s with a 300ms crossfade — fine on a browser tab you're looking straight at, distracting on a screen sitted across a room from. Login screen backdrops here use 60s display / 10s crossfade instead, found by trying the literal web timing first and getting real feedback that it didn't work for the medium.
